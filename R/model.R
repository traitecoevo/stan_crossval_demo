...
stan_model_re <- function() {
  list(
    pars = c('alpha_mu','beta_mu','alpha_sigma','beta_sigma','sigma_spp','sigma_site','sigma_obs','loglik_heldout'),
    model = "
    data {
      int<lower = 1> n_obs; 
      int<lower = 1> n_site;
      ...
      real ln_ht[n_obs];
      int<lower = 1> n_obs_heldout; 
      int<lower = 1> n_site_heldout;
      int<lower = 1> n_spp_heldout;
      int<lower = 1> site_heldout[n_obs_heldout]; 
  ...
  model {
    ...
    for (i in 1:n_obs) {
      y_hat[i] <- alpha[spp[i]] + beta[spp[i]] * ln_alf[i] + site_effect[site[i]];
    }
    ln_ht ~ normal(y_hat, sigma_obs);
    
    # Priors
    alpha ~ normal(alpha_mu,alpha_sigma);
    beta ~ normal(alpha, 10);
  ...
  generated quantities{
    ...
    for (i in 1:n_obs_heldout) {
      y_hat_heldout[i] <- alpha[spp_heldout[i]] + beta[spp_heldout[i]] * ln_alf_heldout[i] + site_effect[site_heldout[i]];
